...
export async function POST(req) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Subject, batch, semester, date, time and who taught the class are required",
        },
        { status: 400 },
      );
    }
    ...
    // Prevent duplicate logs: same assignment, batch, semester, date, timeslot and teacher
    ...
    const existing = await LectureLog.findOne({
      subjectAssignmentId,
      batchId,
      semesterNumber,
      startTime,
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "This lecture has already been logged for the specified timeslot",
        },
        { status: 409 },
      );
    }
