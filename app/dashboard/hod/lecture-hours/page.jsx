    ...
    <div className="bg-white border border-gray-100 rounded-xl p-4">
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((day, idx) => {
          ...
          return (
            <button key={idx} onClick={() => onSelect(cellDate)}
...
function LogForm({ selectedDate, batches, onSaved }) {
  ...
  const { assignments } = useAssignments(batchId, semesterNumber ? Number(semesterNumber) : null)

  const selectedAssignment = assignments.find(a => a._id === form.subjectAssignmentId)
  ...
  async function handleSubmit(e) {
    ...
    if (!batchId || !semesterNumber || !form.subjectAssignmentId || !form.startTime || !form.endTime || !form.taughtBy) {
      setError('Please fill in batch, semester, subject, times, and who taught the class')
      return
    }
    ...
    try {
      ...
      setForm({ subjectAssignmentId: '', startTime: '', endTime: '', taughtBy: '', notes: '' })
      setSuccess(true)
      onSaved()
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
      <p className="text-sm font-semibold text-gray-700">
        Log a lecture for {selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
      </p>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-gray-500 block mb-1">Batch</label>
          <select value={batchId} onChange={e => { setBatchId(e.target.value); setSemesterNumber('') }}
            ...
            {batches.map(b => <option key={b._id} value={b._id}>{b.name} ({b.programme})</option>)}
          </select>
        </div>
        <div>
          <label className="text-xs text-gray-500 block mb-1">
            Semester {selBatchObj && (
              <span className="text-gray-300">
                ({selBatchObj.programme === 'BSc' ? '4 total' : selBatchObj.programme === 'BCS' ? '2 total' : ''})
              </span>
            )}
          </label>
          <select value={semesterNumber} onChange={e => setSemesterNumber(e.target.value)} disabled={!batchId}
      ...
      <div>
        ...
        <select name="subjectAssignmentId" value={form.subjectAssignmentId} onChange={handleFormChange}
      ...
      <div>
        <label className="text-xs text-gray-500 block mb-1">Who taught this class?</label>
        {selectedAssignment ? (
          selectedAssignment.lecturerId ? (
            <div className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-700 flex items-center justify-between">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[selectedAssignment.lecturerId.role]}`}>
                {ROLE_LABELS[selectedAssignment.lecturerId.role] || selectedAssignment.lecturerId.role}
              </span>
            </div>
          ) : (
            <div className="border border-amber-200 bg-amber-50 rounded-lg px-3 py-2 text-sm text-amber-700">
              No lecturer assigned to this subject yet — ask a coordinator to assign one before logging hours
            </div>
          )
        ) : (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select name="startTime" value={form.startTime} onChange={handleFormChange}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900">
            <option value="">—</option>
        ...
        <div>
          ...
          <select name="endTime" value={form.endTime} onChange={handleFormChange}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900">
            <option value="">—</option>
            {TIMES.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
      ...
      {success && <div className="bg-green-50 text-green-700 text-sm px-3 py-2 rounded-lg">Lecture logged successfully</div>}

      <button type="submit" disabled={loading || !selectedAssignment?.lecturerId}
  ...
  async function handleDeleteLog(id) {
    ...
    try {
      ...
      if (!res.ok || !data?.success) {
        ...
        alert(data?.message || `Failed to delete (status ${res.status}). Check console for details.`)
        return
      }
      fetchAllLogs()
    } catch (err) {
    ...
    <div className="grid grid-cols-[280px_1fr_340px] gap-5">
      ...
      <div>
        <div className="bg-white border border-gray-100 rounded-xl p-5 mb-4">
          ...
          {loadingAll ? (
            ...
            <div className="text-center py-8">
              <p className="text-sm text-gray-400">No lectures logged for this day yet</p>
            </div>
          ) : (
            <div className="space-y-2">
              {dayLogs.map(log => (
                <div key={log._id} className="flex items-start justify-between border border-gray-100 rounded-lg px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {log.startTime} – {log.endTime} &nbsp;·&nbsp; {log.durationHours} hrs
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      Taught by <span className="text-gray-600 font-medium">{log.taughtBy?.name}</span>
                      <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full font-medium ${ROLE_COLORS[log.taughtBy?.role]}`}>
                        {ROLE_LABELS[log.taughtBy?.role] || log.taughtBy?.role}
                      </span>
                      {log.loggedBy?._id !== log.taughtBy?._id && (
                        <span className="text-gray-400"> &nbsp;·&nbsp; logged by {log.loggedBy?.name}</span>
                      )}
                    </p>
                    {log.notes && (
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-800 leading-relaxed">
          Anyone can log a lecture session — students, lecturers, coordinators, and the HOD.
          Entries count immediately and feed into subject hour totals and visiting lecturer
          payment calculations. Double-check the subject, date, time and lecturer before saving.
        </div>
      </div>

      <div>
        <LogForm selectedDate={selectedDate} batches={batches} onSaved={fetchAllLogs} />
      </div>
...
function SummaryTab({ batches }) {
  ...
  async function fetchSummary() {
    ...
    setLoading(true)
    const res  = await fetch(`/api/lecture-logs/summary?batchId=${batchId}&semesterNumber=${semesterNumber}`)
    const data = await res.json()
    if (data.success) {
  ...
  useEffect(() => { fetchSummary() }, [batchId, semesterNumber])
    ...
    <div>
      ...
      {!batchId || !semesterNumber ? (
        <div className="bg-white border border-dashed border-gray-200 rounded-xl p-12 text-center">
          <p className="text-sm text-gray-400">Select a batch and semester to view hour totals</p>
        </div>
      ) : (
        <>
          <div className="flex gap-2 mb-5 bg-blue-50/60 border border-blue-100 rounded-xl p-1.5 w-fit">
            <button onClick={() => setView('lecturer')}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left px-5 py-3 text-xs text-gray-400 font-medium">Sessions</th>
                    <th className="text-left px-5 py-3 text-xs text-gray-400 font-medium">Total hours</th>
                  </tr>
                </thead>
                <tbody>
                  {byLecturer.length === 0 ? (
                    ...
                    <tr key={row.lecturerId} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="px-5 py-3 font-medium text-gray-800">{row.name}</td>
                      <td className="px-5 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[row.role]}`}>
                          {ROLE_LABELS[row.role] || row.role}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-gray-500">{row.sessionCount}</td>
                      <td className="px-5 py-3 font-semibold text-gray-800">{row.totalHours.toFixed(1)} hrs</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left px-5 py-3 text-xs text-gray-400 font-medium">Assigned to</th>
                    <th className="text-left px-5 py-3 text-xs text-gray-400 font-medium">Sessions</th>
                    <th className="text-left px-5 py-3 text-xs text-gray-400 font-medium">Total hours</th>
                  </tr>
                </thead>
                <tbody>
                  {bySubject.length === 0 ? (
                    ...
                    <tr key={row.subjectAssignmentId} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="px-5 py-3">
                        <span className="font-mono font-medium text-gray-800">{row.subjectCode}</span>
                        <span className="text-gray-400 ml-2">{row.subjectName}</span>
                      </td>
                      <td className="px-5 py-3 text-gray-500">{row.credits}</td>
                      <td className="px-5 py-3 text-gray-500">{row.assignedLecturer || '—'}</td>
                      <td className="px-5 py-3 text-gray-500">{row.sessionCount}</td>
                      <td className="px-5 py-3 font-semibold text-gray-800">{row.totalHours.toFixed(1)} hrs</td>
                    </tr>
                  ))}
  ...
  return (
    <div>
      ...
      <div className="px-8 py-6">
        ...
        {tab === 'log' ? <LogTab batches={batches} />
          : tab === 'summary' ? <SummaryTab batches={batches} />
          : <LecturerListTab />}
      </div>
    </div>
  )
