...
const SUBMISSION_DOCS = [
  {
    ...
    label: 'Research Proposal Format',
  },
  {
  ...
  },
  {
    ...
    label: 'Final Thesis Format',
  },
  {
    file: 'Annex_02_Interim_Report_Format.docx',
    label: 'Interim Report Format',
  },
]
  ...
  function renderCards(list) {
    ...
    return list.map((s) => {
      ...
      return (
        <div
          key={s._id}
          onClick={() => router.push(`/dashboard/hod/projects/${s._id}`)}
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium border ${PROG_COLORS[s.programme]}`}>
              {s.programme}
            </span>
        ...
        <div className="mt-8">
          <div className="bg-white border border-gray-100 rounded-xl p-4">
            <h4 className="text-sm font-semibold mb-4">
              Submission documents for final research project
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {SUBMISSION_DOCS.map((doc) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-gray-400">{doc.file}</div>
                  </div>
                  <div className="text-xs text-blue-600">⬇ Download</div>
                </a>
              ))}
            </div>
          </div>
