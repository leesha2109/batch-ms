...
const MILESTONES = [
  { key: 'titleConfirmed',    label: 'Confirm project title',   desc: 'Project title finalised and approved'         },
...
function buildMarkRows(project) {
  ...
  if (!students.length) {
    return [{ name: '', studentId: '', supervisor: project?.supervisorId?.name || '', mark: 0, m1: 0, m2: 0, m3: 0, m4: 0, reportMarks: 0 }]
  }
  return students.map(student => ({
    name:       student.name      || '',
    studentId:  student.studentId || '',
    supervisor: project?.supervisorId?.name || '',
    mark: 0, m1: 0, m2: 0, m3: 0, m4: 0, reportMarks: 0,
  }))
}
...
const TH = 'px-3 py-3 text-left text-xs font-bold text-gray-700 border border-gray-200 bg-gray-50'
const TH_C = 'px-3 py-3 text-center text-xs font-bold text-gray-700 border border-gray-200 bg-gray-50'
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h2 className="text-base font-semibold">{isEditing ? 'Edit project' : 'Add new project'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
              rows={2} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none"/>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500 block mb-1">Type</label>
              <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value, students: [] }))}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
          ...
          <div>
            <label className="text-xs text-gray-500 block mb-1">
              {form.type === 'group' ? 'Assign students (multiple)' : 'Assign student (one)'}
            </label>
            <div className="border border-gray-200 rounded-lg max-h-40 overflow-y-auto divide-y divide-gray-50">
              {students.length === 0
                ...
                : students.map(s => (
                  <label key={s._id} className="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 cursor-pointer">
                    <input
                      type={form.type === 'individual' ? 'radio' : 'checkbox'}
                      checked={form.students.includes(s._id)}
                      onChange={() => form.type === 'individual'
...
function ProjectMarksPanel({ project, onProjectUpdated }) {
  ...
  const [marksState, setMarksState] = useState(project?.marks || buildDefaultMarks(project))
  ...
  function saveMarks(nextState) {
    ...
    fetch(`/api/projects/${project._id}`, {
      ...
      .then(readJsonSafely)
      .then(data => { if (data?.success && data.project && onProjectUpdated) onProjectUpdated(data.project) })
    ...
    const nextState = {
      ...
      [sheetKey]: {
        ...
        rows: (marksState?.[sheetKey]?.rows || []).map((row, idx) =>
          idx !== rowIndex ? row : { ...row, mark: Number.isFinite(clean) ? clean : 0 }
        )
      }
    }
  ...
  function updateProposalRow(rowIndex, field, value) {
    ...
    const nextRows = (marksState?.proposal?.rows || []).map((row, idx) => {
      ...
      const updated = { ...row, [field]: Number.isFinite(clean) ? Math.min(clean, 100) : 0 }
      const avg = (Number(updated.m1||0)+Number(updated.m2||0)+Number(updated.m3||0)+Number(updated.m4||0)) / 4
      return { ...updated, mark: Number(Math.min(avg + Number(updated.reportMarks||0), 10).toFixed(2)) }
    })
    const nextState = { ...marksState, proposal: { ...(marksState?.proposal||{max:10}), rows: nextRows } }
    setMarksState(nextState)
    saveMarks(nextState)
  }
  ...
  function updateFinalReportRow(rowIndex, field, value) {
    ...
    const nextRows = (marksState?.finalReport?.rows || []).map((row, idx) => {
      if (idx !== rowIndex) return row
      const updated = { ...row, [field]: Number.isFinite(clean) ? Math.min(clean, 100) : 0 }
      const filled  = [updated.m1, updated.m2].filter(v => v !== '' && v !== undefined && v !== 0).length
      const avg     = filled > 0 ? (Number(updated.m1||0) + Number(updated.m2||0)) / filled : 0
      const scaled  = Number(((avg / 100) * 45).toFixed(2))
      return { ...updated, mark: scaled }
    })
    const nextState = { ...marksState, finalReport: { ...(marksState?.finalReport||{max:45}), rows: nextRows } }
    setMarksState(nextState)
    saveMarks(nextState)
  }
  ...
  function updateFinalVivaRow(rowIndex, field, value) {
    ...
    const nextState = { ...marksState, finalViva: { ...(marksState?.finalViva||{max:20}), rows: nextRows } }
    setMarksState(nextState)
    saveMarks(nextState)
  }
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          {ALL_MARK_TABS.map(sheet => (
            <button key={sheet.key} type="button" onClick={() => setActiveTab(sheet.key)}
            ...
            <table className="min-w-full text-sm border-collapse">
              ...
              <tbody>
                {(marksState?.finalReport?.rows || []).map((row, rowIndex) => {
                  ...
                  const filled = [row.m1, row.m2].filter(v => v !== '' && v !== undefined && Number(v) !== 0).length
                  const avg    = filled > 0 ? (m1 + m2) / filled : 0
                  const scaled = Number(((avg / 100) * 45).toFixed(2))
                  return (
                    <tr key={`fr-${rowIndex}`} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                      <td className={TD}>{row.name || '—'}</td>
                      <td className={`${TD} font-mono text-blue-600`}>{row.studentId || '—'}</td>
                      <td className={TD}>{project?.title || '—'}</td>
                      <td className={TD}>{row.supervisor || project?.supervisorId?.name || '—'}</td>
                      {/* M1 — yellow editable */}
                      <td className="px-2 py-2 border border-gray-200 text-center bg-yellow-50">
                        <YellowInput
                          value={row.m1 || ''}
                          onChange={v => updateFinalReportRow(rowIndex, 'm1', v)}
                        />
                      </td>
                      {/* M2 — yellow editable */}
                      <td className="px-2 py-2 border border-gray-200 text-center bg-yellow-50">
                        <YellowInput
                          value={row.m2 || ''}
                          onChange={v => updateFinalReportRow(rowIndex, 'm2', v)}
                        />
                      </td>
          ...
          <table className="min-w-full text-sm border-collapse">
            ...
            <tbody>
              {(marksState?.proposal?.rows || []).map((row, rowIndex) => {
                const avg   = (Number(row.m1||0)+Number(row.m2||0)+Number(row.m3||0)+Number(row.m4||0)) / 4
                const total = Number(Math.min(avg + Number(row.reportMarks||0), 10).toFixed(2))
                return (
                  <tr key={`proposal-${rowIndex}`} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                    <td className={TD}>{row.name || '—'}</td>
                    <td className={`${TD} font-mono text-blue-600`}>{row.studentId || '—'}</td>
                    <td className={TD}>{project?.title || '—'}</td>
                    <td className={TD}>{row.supervisor || project?.supervisorId?.name || '—'}</td>
                    {['m1','m2','m3','m4'].map(f => (
                      <td key={f} className="px-2 py-2 border border-gray-200 text-center bg-yellow-50">
                        <YellowInput value={row[f] || ''} onChange={v => updateProposalRow(rowIndex, f, v)} />
                      </td>
                    ))}
                    ...
                    <td className="px-2 py-2 border border-gray-200 text-center bg-yellow-50">
                      <YellowInput value={row.reportMarks || ''} max={5} onChange={v => updateProposalRow(rowIndex, 'reportMarks', v)} />
                    </td>
                    <td className="px-3 py-2 border border-gray-200 text-center bg-green-50">
            ...
            <table className="min-w-full text-sm border-collapse">
              <thead>
                <tr>
                  ...
                  <th className={`${TH_C} bg-yellow-100 ring-2 ring-inset ring-green-600`}>m4</th>
              ...
              <tbody>
                {(marksState?.finalViva?.rows || []).map((row, rowIndex) => {
                  ...
                  return (
                    <tr key={`viva-${rowIndex}`} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                      <td className={TD}>{row.name || '—'}</td>
                      <td className={`${TD} font-mono text-blue-600`}>{row.studentId || '—'}</td>
                      <td className={TD}>{project?.title || '—'}</td>
                      <td className={TD}>{row.supervisor || project?.supervisorId?.name || '—'}</td>
                      {['m1','m2','m3'].map(f => (
                        <td key={f} className="px-2 py-2 border border-gray-200 text-center bg-yellow-50">
                          <YellowInput value={row[f] || ''} onChange={v => updateFinalVivaRow(rowIndex, f, v)} />
                        </td>
                      ))}
                      <td className="px-2 py-2 border border-gray-400 text-center bg-yellow-100 ring-1 ring-inset ring-green-600">
                        <input type="number" min="0" max="100"
                          value={row.m4 || ''}
                          onChange={e => updateFinalVivaRow(rowIndex, 'm4', e.target.value)}
          ...
          <table className="min-w-full text-sm border-collapse">
            <thead>
              <tr>
                ...
                <th className={TH}>Supervisor/s</th>
                <th className={TH_C}>
                  Mark ({marksState?.[activeTab]?.max ?? MARK_SHEETS.find(s => s.key === activeTab)?.max})
                </th>
              </tr>
            </thead>
            <tbody>
              {(marksState?.[activeTab]?.rows || []).map((row, rowIndex) => (
                <tr key={`${activeTab}-${rowIndex}`} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                  <td className={TD}>{row.name || '—'}</td>
                  <td className={`${TD} font-mono text-blue-600`}>{row.studentId || '—'}</td>
                  <td className={TD}>{project?.title || '—'}</td>
                  <td className={TD}>{row.supervisor || project?.supervisorId?.name || '—'}</td>
      ...
      {activeTab === 'finalSummary' && (
        <div className="overflow-x-auto border border-gray-200 rounded-xl">
          <table className="min-w-full text-sm border-collapse">
            ...
            <tbody>
              {(marksState.proposal?.rows || []).map((row, rowIndex) => {
                const proposal    = Number(marksState.proposal?.rows?.[rowIndex]?.mark    || 0)
                const interim     = Number(marksState.interim?.rows?.[rowIndex]?.mark     || 0)
                const finalReport = Number(marksState.finalReport?.rows?.[rowIndex]?.mark || 0)
                const publication = Number(marksState.publication?.rows?.[rowIndex]?.mark || 0)
                const finalViva   = Number(marksState.finalViva?.rows?.[rowIndex]?.mark   || 0)
                const total       = proposal + interim + finalReport + publication + finalViva
                ...
                const gradeColor = ['A+','A','A-'].includes(grade) ? 'text-green-700'
                  : ['B+','B','B-'].includes(grade) ? 'text-blue-700'
                  : grade === 'C+' ? 'text-yellow-700' : 'text-red-600'
                return (
                  <tr key={`summary-${rowIndex}`} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/40'}>
                    <td className={TD}>{row.name || '—'}</td>
                    <td className={`${TD} font-mono text-blue-600`}>{row.studentId || '—'}</td>
                    <td className={TD}>{project?.title || '—'}</td>
                    <td className={TD}>{row.supervisor || project?.supervisorId?.name || '—'}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{proposal.toFixed(2)}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{interim.toFixed(2)}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{finalReport.toFixed(2)}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{publication.toFixed(2)}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{finalViva.toFixed(2)}</td>
                    <td className="px-3 py-3 border border-gray-200 text-center bg-green-50 font-bold text-green-800">{total.toFixed(2)}</td>
                    <td className={`px-3 py-3 border border-gray-200 text-center bg-green-50 font-bold ${gradeColor}`}>{grade}</td>
                  </tr>
                )
              })}
            </tbody>
...
// ── Project detail view ───────────────────────────────────────────────────────
function ProjectDetailView({ project, onBack, onEdit, onDelete, onProjectUpdated }) {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={onEdit} className="text-sm border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-gray-50">Edit</button>
          <button onClick={onDelete} className="text-sm border border-red-100 text-red-500 px-3 py-1.5 rounded-lg hover:bg-red-50">Delete</button>
        </div>
      </div>

      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-2">
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${project.type === 'group' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
        ...
        <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-50">
          <div>
            ...
            <p className="text-sm font-medium text-gray-700">{project.supervisorId?.name || '—'}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-xs text-gray-400 mb-0.5">{project.type === 'group' ? 'Students' : 'Student'}</p>
            <div className="flex flex-wrap gap-1">
              {project.students?.length === 0
                ? <span className="text-sm text-gray-400">—</span>
                : project.students?.map(s => (
                  <span key={s._id} className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-700">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
          ...
          {MILESTONES.map((milestone, idx) => {
            ...
            return (
              <div key={milestone.key}
                ...
                <div className={`flex-shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center ${isChecked ? 'bg-green-500 border-green-500 text-white' : 'bg-white border-gray-200'}`}>
                  {isChecked ? <span className="text-xs font-bold">✓</span> : <span className="text-xs text-gray-400">{idx + 1}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${isChecked ? 'text-green-700' : 'text-gray-500'}`}>{milestone.label}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{milestone.desc}</p>
                </div>
                <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${isChecked ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
        ...
        <div className="mt-5 pt-4 border-t border-gray-50">
          ...
          <div className="w-full bg-gray-100 rounded-full h-2">
            <div className="bg-green-500 h-2 rounded-full transition-all" style={{ width: `${(completedCount / 4) * 100}%` }}/>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">
          Submitted documents
          <span className="ml-2 text-xs font-normal text-gray-400">{uploadedDocs.length} / {EXPECTED_DOCS.length} uploaded</span>
        </h3>
        <div className="grid grid-cols-2 gap-3">
              ...
              <div key={expected.key}
                ...
                {uploaded && (
                  <a href={uploaded.url} target="_blank" rel="noreferrer" download
                    className="text-xs text-blue-600 hover:underline ml-2 flex-shrink-0">⬇ Download</a>
                )}
              </div>
            )
          })}
      ...
      <ProjectMarksPanel project={project} onProjectUpdated={onProjectUpdated} />
    </div>
  )
}
  ...
  return (
    <div>
      <TopHeader
        ...
        subtitle={subject ? `${subject.code} · Level ${subject.level} · Semester ${subject.semester} · ${subject.programme}` : ''}
        action={
          ...
          <>
            <div className="flex gap-3 mb-6 items-center flex-wrap">
              ...
              <select value={selBatch} onChange={e => setSelBatch(e.target.value)}
                ...
                {matchingBatches.map(b => <option key={b._id} value={b._id}>{b.name}</option>)}
              </select>
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <div className="border-b border-gray-200 bg-gray-50 px-4 py-3 flex flex-wrap gap-2">
                  {ALL_MARK_TABS.map(tab => (
                    <button key={tab.key} type="button" onClick={() => setMarkTab(tab.key)}
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-gray-50 border-b-2 border-gray-200">
                        ...
                        {markTab === 'proposal' && (
                          <>
                            <th className={TH_C}>M1</th><th className={TH_C}>M2</th>
                            <th className={TH_C}>M3</th><th className={TH_C}>M4</th>
                            <th className={TH_C}>Avg PPT (5)</th>
                            <th className={TH_C}>Report (5)</th>
                            <th className={TH_C}>Total (10)</th>
                          </>
                        ...
                        {!['proposal','finalReport','finalViva','finalSummary'].includes(markTab) && (
                          <th className={TH_C}>
                            Mark ({MARK_SHEETS.find(s => s.key === markTab)?.max || 0})
                          </th>
                        )}
                      </tr>
                    </thead>
                    <tbody>
                      {projects.flatMap(p => {
                        const projectStudents = p.students?.length ? p.students : [{ _id: p._id, name: '—', studentId: '—' }]
                        return projectStudents.map((student, si) => {
                          const marks       = p.marks || buildDefaultMarks(p)
                          const proposal    = Number(marks.proposal?.rows?.[si]?.mark    || 0)
                          const interim     = Number(marks.interim?.rows?.[si]?.mark     || 0)
                          const finalReport = Number(marks.finalReport?.rows?.[si]?.mark || 0)
                          const publication = Number(marks.publication?.rows?.[si]?.mark || 0)
                          const finalViva   = Number(marks.finalViva?.rows?.[si]?.mark   || 0)
                          const total       = proposal + interim + finalReport + publication + finalViva
                          const rounded     = Math.round(total)
                          let grade = 'F'
                          if (rounded >= 75) grade = 'A+'
                          ...
                          const frRow  = marks.finalReport?.rows?.[si] || { m1: 0, m2: 0 }
                          const frFilled = [frRow.m1, frRow.m2].filter(v => v !== '' && v !== undefined && Number(v) !== 0).length
                          const frAvg  = frFilled > 0 ? (Number(frRow.m1||0) + Number(frRow.m2||0)) / frFilled : 0
                          const frScaled = Number(((frAvg / 100) * 45).toFixed(2))

                          const vivaRow  = marks.finalViva?.rows?.[si] || { m1:0, m2:0, m3:0, m4:0 }
                          const vivaAvg  = (Number(vivaRow.m1||0)+Number(vivaRow.m2||0)+Number(vivaRow.m3||0)+Number(vivaRow.m4||0)) / 4
                          const vivaTotal = Number(((vivaAvg/100)*20).toFixed(2))

                          const propRow  = marks.proposal?.rows?.[si] || { m1:0, m2:0, m3:0, m4:0, reportMarks:0 }
                          const propAvg  = (Number(propRow.m1||0)+Number(propRow.m2||0)+Number(propRow.m3||0)+Number(propRow.m4||0)) / 4
                          const propTotal = Number(Math.min(propAvg + Number(propRow.reportMarks||0), 10).toFixed(2))

                          const gradeColor = ['A+','A','A-'].includes(grade) ? 'text-green-700 font-bold'
                            : ['B+','B','B-'].includes(grade) ? 'text-blue-700 font-bold'
                            : grade === 'C+' ? 'text-yellow-700 font-bold' : 'text-red-600 font-bold'

                          return (
                            <tr key={`${p._id}-${student._id||si}`} className="border-b border-gray-200 hover:bg-blue-50/30">
                              <td className={TD}>{student.name||'—'}</td>
                              <td className={`${TD} font-mono text-blue-600`}>{student.studentId||'—'}</td>
                              ...
                              {markTab === 'finalReport' && (
                                <>
                                  <td className="px-3 py-3 border border-gray-200 text-center bg-yellow-50 text-sm">{frRow.m1||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center bg-yellow-50 text-sm">{frRow.m2||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center bg-green-50 font-semibold text-green-800">{frFilled > 0 ? frScaled : '—'}</td>
                                </>
                              )}
                              {markTab === 'proposal' && (
                                <>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{propRow.m1||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{propRow.m2||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{propRow.m3||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{propRow.m4||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center font-semibold">{propAvg.toFixed(2)}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{propRow.reportMarks||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center font-semibold">{propTotal}</td>
                                </>
                              )}
                              {markTab === 'finalViva' && (
                                <>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{vivaRow.m1||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{vivaRow.m2||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{vivaRow.m3||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center">{vivaRow.m4||0}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center font-semibold">{vivaAvg.toFixed(2)}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center font-semibold">{vivaTotal}</td>
                                </>
                              )}
                              {markTab === 'finalSummary' && (
                                <>
                                  ...
                                  <td className="px-3 py-3 border border-gray-200 text-center bg-blue-50/50">{finalViva.toFixed(2)}</td>
                                  <td className="px-3 py-3 border border-gray-200 text-center bg-green-50 font-bold text-green-800">{total.toFixed(2)}</td>
                                  <td className={`px-3 py-3 border border-gray-200 text-center bg-green-50 ${gradeColor}`}>{grade}</td>
                                </>
                              )}
                              {!['proposal','finalReport','finalViva','finalSummary'].includes(markTab) && (
                                <td className="px-3 py-3 border border-gray-200 text-center font-semibold">
                                  {markTab === 'interim' ? interim : markTab === 'publication' ? publication : 0}
                                </td>
                              )}
                            </tr>
                          )
                    ...
                    {projects.flatMap(p => {
                      ...
                      return projectStudents.map((student, si) => (
                        <tr key={`${p._id}-${student._id||si}`}
                          ...
                          <td className={TD}>{p.title||'—'}</td>
                          <td className={TD}>{p.supervisorId?.name||'—'}</td>
                          <td className="px-4 py-4 border border-gray-200 text-center" onClick={e => e.stopPropagation()}>
      ...
      {showModal && (
        <ProjectFormModal
          ...
          onSaved={() => { fetchProjects(); setDetailProject(null) }}
        />
      )}
    </div>
  )
