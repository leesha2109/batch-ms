...
export default function ResultsPage() {
  ...
  function handleExamChange(a, value) {
    ...
    patchAssignment(
      a._id,
      held ? { examHeld: true } : { examHeld: false, resultsReleased: false },
    );
  }

  function handleResultsChange(a, value) {
    if (!a.examHeld) return;
        ...
        {selBatch && selSemester ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${TYPE_COLORS[a.subjectId?.type]}`}
                          >
                            {a.subjectId?.type}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-3">
                          <select
                            ...
                            onChange={(e) =>
                              handleExamChange(a, e.target.value)
                            }
                            className={`text-s px-2 py-1 rounded-full font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-900 disabled:opacity-50 ${EXAM_COLORS[examHeld ? "held" : "not_held"]}`}
                          >
                        ...
                        <td className="px-5 py-3">
                          <select
                            ...
                            onChange={(e) =>
                              handleResultsChange(a, e.target.value)
                            }
                            className={`text-s px-2 py-1 rounded-full font-medium border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-900 disabled:opacity-50 ${RESULT_COLORS[released ? "released" : "pending"]}`}
                          >
          ...
          <div className="bg-white border border-dashed border-blue-200 rounded-xl p-10 text-center">
            ...
            <p className="text-sm text-blue-400">
              Select a batch and semester to view exam & results status
            </p>
          </div>
        )}
      </div>
