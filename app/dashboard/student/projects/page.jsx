...
const MILESTONES = [
  {
    ...
    desc: "Your project title has been finalised and approved",
  },
  {
...
const REQUIRED_DOCS = [
  {
    ...
    file: "Annex_01_Research_proposal_format.docx",
  },
  {
    key: "interim_report",
    label: "Interim Report",
    file: "Annex_02_Interim_Report_Format.docx",
  },
  {
    key: "manuscript",
    label: "Manuscript",
    file: "Annex_03_Manuscrip_format.docx",
  },
  {
    key: "final_thesis",
    label: "Final Thesis",
    file: "Annex_04_Final_THESIS_Format.docx",
  },
];
...
function DocSlot({ doc, uploadedDoc, projectId, onUploaded }) {
  ...
  const [localUploadedDoc, setLocalUploadedDoc] = useState(uploadedDoc);

  useEffect(() => {
    setLocalUploadedDoc(uploadedDoc);
  }, [uploadedDoc]);
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      setLocalUploadedDoc(data.document || uploadedDoc || null);
  ...
  async function handleRemove() {
    const activeDoc = localUploadedDoc || uploadedDoc;
    if (!activeDoc || !confirm("Remove this document?")) return;
    await fetch(`/api/projects/${projectId}/documents`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ docUrl: activeDoc.url }),
      credentials: "include",
    });
    setLocalUploadedDoc(null);
    onUploaded();
  }
  ...
  return (
    <div
      ...
      ${activeDoc ? "border-green-200 bg-green-50" : "border-gray-200 bg-white"}`}
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-lg">{activeDoc ? "📄" : "📭"}</span>
          <div>
      ...
      <label
        className={`flex items-center justify-center gap-2 border-2 border-dashed rounded-lg p-2.5 cursor-pointer transition-colors
        ${
          uploading
...
function ProjectDetail({ project, onBack, onUploaded }) {
  ...
  const docsByKey = Object.fromEntries(
    docs.filter((doc) => doc.key).map((doc) => [doc.key, doc]),
  );
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">
            {Object.keys(docsByKey).length || docs.length} /{" "}
        ...
        <div className="grid grid-cols-2 gap-3">
          {REQUIRED_DOCS.map((doc) => (
            <DocSlot
              key={doc.key}
              doc={doc}
              uploadedDoc={docsByKey[doc.key] || null}
...
export default function StudentProjectsPage() {
    ...
    try {
      ...
      if (data.success) {
        const filtered = (data.projects || []).filter(
          (p) => !p.subjectId || p.subjectId.type === "project",
        );
  ...
  async function handleUploaded(updatedProject = null) {
    if (updatedProject) {
          ...
          <>
            ...
            <div className="bg-white border border-gray-100 rounded-xl p-5">
              ...
              <p className="text-xs text-gray-400 mb-4">
                Download the required format templates before submitting your
                documents.
              </p>
              <div className="grid grid-cols-2 gap-3">
                {REQUIRED_DOCS.map((doc) => (
