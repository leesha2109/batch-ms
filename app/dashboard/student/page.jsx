...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (!semesterNumber) {
      ...
      fetch("/api/student/timetable", { credentials: "include" })
        .then((r) => r.json())
        .then((d) => {
          if (d.success && d.batch) setBatch(d.batch);
        });
      return;
    }
  ...
  assignments.forEach((a, i) => {
    colorMap[String(a._id)] = COLORS[i % COLORS.length];
  });
    ...
    slots.forEach((slot) => {
      ...
      let next = slots.find(
        (s) =>
          s.day === slot.day &&
          s.startTime === endTime &&
          String(s.subjectAssignmentId?._id || s.subjectAssignmentId || "") ===
            slotId,
      );
      while (next) {
    ...
    const gridRows = TIMES.slice(0, -1)
      .map((time) => {
        const cells = DAYS.map((day) => {
          const isCovered = mergedSlots.some(
            (s) =>
              s.day === day &&
      ...
      <div className="px-8 py-6">
        ...
        {!batch ? (
          ...
          <div className="bg-white border border-dashed border-gray-200 rounded-xl p-12 text-center">
            <p className="text-3xl mb-3">📅</p>
            <p className="text-sm text-gray-400">
              No timetable published yet for this semester.
            </p>
          </div>
        ) : (
          <div ref={timetableRef} className="bg-white p-4 rounded-xl">
                        ...
                        {mergedSlots
                          ...
                          .map((slot) => {
                            ...
                            const color = assignment
                              ? colorMap[String(assignment._id)]
            ...
            <p className="text-xs text-gray-400 mt-3">
              Contact your coordinator if you notice any errors in the
              timetable.
            </p>
          </div>
        )}
