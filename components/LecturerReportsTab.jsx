    ...
    <div id="lecturer-print-area" className="hidden print:block">
      {subjectGroups.map((group, idx) => (
        <div
          key={idx}
          style={{
            pageBreakAfter: idx < subjectGroups.length - 1 ? "always" : "auto",
          }}
          className="p-10 font-serif text-black"
        >
          <table className="w-full border-collapse border border-black mb-0 text-sm">
            <tbody>
              <tr>
                ...
                <td className="border border-black px-2 py-1 w-1/2">
                  {lecturer.name}
                </td>
              </tr>
            </tbody>
          ...
          <table className="w-full border-collapse border border-black text-sm">
            <thead>
              <tr>
                <th
                  colSpan={4}
                  className="border border-black bg-blue-100 px-2 py-1 font-medium"
                >
                  {group.subjectCode} - {group.subjectName} &nbsp;·&nbsp;{" "}
                  {group.batchName} &nbsp;·&nbsp; Semester{" "}
                  {group.semesterNumber}
                </th>
              </tr>
              <tr className="bg-blue-100">
                <th className="border border-black px-2 py-1 font-medium w-24">
                  Lecture No.
                </th>
                <th className="border border-black px-2 py-1 font-medium w-28">
                  Date
                </th>
                <th className="border border-black px-2 py-1 font-medium">
                  Time
                </th>
                <th className="border border-black px-2 py-1 font-medium w-32">
                  Signature
                </th>
              </tr>
            </thead>
            <tbody>
              ...
              {Array.from({
                length: Math.max(0, 14 - group.sessions.length),
              }).map((_, i) => (
                <tr key={`blank-${i}`}>
                  <td className="border border-black px-2 py-1.5">&nbsp;</td>
                  <td className="border border-black px-2 py-1.5">&nbsp;</td>
              ...
              <tr>
                <td
                  colSpan={2}
                  className="border border-black px-2 py-1.5 font-medium text-right"
                >
                  Total hours
                </td>
                <td
          ...
          <p className="mt-8 leading-relaxed">
            ...
            above schedule{" "}
            {dateRangeLabel ? `in the period of ${dateRangeLabel}` : ""} and
            that I have not applied for the payment for the above lectures and I
            have not received any payment for the same.
          </p>
          ...
          <div className="mt-16 flex justify-end">
            <div className="text-center">
              <p className="border-t border-black pt-1 px-10">
                Signature of the{" "}
                {lecturer.role === "visiting_lecturer" ? "visiting" : ""}{" "}
                Lecturer
              </p>
            </div>
          </div>
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 text-sm text-gray-400">
          Loading report...
        </div>
      </div>
    );
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((l) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${ROLE_COLORS[l.role]}`}
                      >
                        {ROLE_LABELS[l.role]}
                      </span>
                    </td>
